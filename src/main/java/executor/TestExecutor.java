		...
		try {
			...
			while (true) {
				try {
					...
					GeneralHelper.CreateFolderToPrints(_testToExecute);
					...
					for (ActionData action : actions) {
						try {
							...
							// TODO add captura de screenshot sempre que falhar
							_screenshotHelper.takeScreenshot();

							// _statusHelper.setStatusErrorOnExecution(_testToExecute);
							break;
						}
						procIndex++;
					}
