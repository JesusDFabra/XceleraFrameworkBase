...
public class GeneralHelper {
	...
	public static String getPrintPath(ExecutionData testToExecute) throws Exception {
		return buildPath(testToExecute) + now() + ".png"; //$NON-NLS-1$
	}

	private static String buildPath(ExecutionData testToExecute) throws Exception {
		ConfigFileReader reader = new ConfigFileReader("configs/config.properties");
		String separator = System.getProperty("file.separator");
		String dirDefaultFolderPrints = reader.getPropertyByKey("dirDefaultPrints");
		if (!isWindows) {
			// Ajuste para trabalhar com LINUX ou DARWIN
			dirDefaultFolderPrints = System.getProperty("user.home") + separator
					+ reader.getPropertyByKey("dirDefaultPrints").replace("c:\\", "").replace("\\", "/");
		}
		return dirDefaultFolderPrints + testToExecute.ExecutionPlan.ExecutionPlanId + " - "
				...
				+ testToExecute.TestPlan.Name + separator + testToExecute.TestSuite.TestSuiteId + " - "
				+ testToExecute.TestSuite.Name + separator + testToExecute.TestCaseId + " - "
				+ testToExecute.TestCaseName.replaceAll("[^a-zA-Z0-9]", "") + separator;
	}
	...
	public static String convertImageInByte(BufferedImage image) throws Exception {
		...
		String encodedImage = new String(Base64.encode(outputStream.toByteArray()), "UTF-8");
	...
	public static void CreateFolderToPrints(ExecutionData testToExecute) throws Exception {

		String path = buildPath(testToExecute);
