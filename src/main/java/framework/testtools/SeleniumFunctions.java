...
public class SeleniumFunctions implements ITestToolFunctions {
	...
	public boolean disabled(String locator) throws Exception {
		...
		if (visible(locator) && !element.isEnabled())
			return true;
...
//					+ File.separator + "extension_2_3_164_0.crx"));
	...
	public void showElements(){
		System.out.printf(_driver.getPageSource());
	}
	...
	public void switchToFrameWithElement(String locator) throws Exception {
		...
		//de prueba... por hacer
		int size = _driver.findElements(By.tagName("frame")).size();
		System.out.println("Total Frames --" + size);
		_driver.switchTo().frame(1); // Switching the Outer Frame

		System.out.println(_driver.getPageSource());

//		size = _driver.findElements(By.tagName("frame")).size();
...
//		System.out.println(_driver.getPageSource());
	...
	@Override
	public void waitObjectToVanish(String locator, int timeOutInSeconds) throws Exception, TimeoutException {
	...
	@Override
	public void WaitObjectVisibility(String locator, int timeOutInSeconds) throws Exception, TimeoutException {
		WebDriverWait wait = new WebDriverWait(_driver, timeOutInSeconds);
		String[] locatorArray = locator.split(":=");
	...
	public void switchWindowSinistro() throws Exception {
		...
		for (String handle : aba) {
			if (i == abas-1) {
				_driver.switchTo().window(handle);
				System.out.println("Navegador actual: index " + i );
				break;
			}
			i++;
		}
